...
const PRESETS: { value: string; labelKey: Keys; ports: number[] }[] = [
  { value: "common", labelKey: "ps.common", ports: [21, 22, 23, 25, 53, 80, 443, 3306, 5432, 6379, 8080] },
  { value: "web", labelKey: "ps.web", ports: [80, 443, 8000, 8080, 8443, 8888, 3000, 5000, 9000] },
  { value: "db", labelKey: "ps.db", ports: [3306, 5432, 5433, 6379, 27017, 1433, 9200] },
];
      ...
      for (const part of customValue.split(",")) {
        ...
        if (a && b) {
          ...
          if (
            ...
            lo >= 1 && hi <= 65535 && lo <= hi &&
            ports.length + (hi - lo + 1) <= 2500
          ) {
            for (let i = lo; i <= hi; i++) ports.push(i);
          }
        } else {
          const p = parseInt(part, 10);
          if (!Number.isNaN(p) && p >= 1 && p <= 65535) ports.push(p);
        }
      }
      return ports;
    }
    ...
    <ToolPage
      ...
      actions={
        <Button variant="primary" leftIcon={<Network size={15} />} onClick={scan} disabled={busy}>
          {busy ? t("ps.scanning") : t("ps.scan")}
        </Button>
      }
      toolbar={
        <>
          <Input className="mono-value" placeholder="localhost" value={host} onChange={(e) => setHost(e.target.value)} style={{ width: 180 }} />
          <SegmentedControl value={preset} onChange={(v) => setPreset(v)} items={[...PRESETS.map((p) => ({ value: p.value, label: t(p.labelKey) })), { value: "custom", label: t("ps.custom") }]} />
        </>
      }
      statusLeft={<span>{t("ps.hint")}</span>}
      statusRight={openPorts ? <span>{t("ps.open", { open: openPorts.length, total: checked.length })}</span> : undefined}
    >
      {preset === "custom" && (
        <input className="fk-input mono-value" placeholder={t("ps.customPlaceholder")} value={custom} onChange={(e) => setCustom(e.target.value)} style={{ marginBottom: 12, width: 260 }} />
      )}
      {openPorts === null ? (
        <EmptyState
        ...
        <EmptyState icon={<Network size={24} />} title={t("ps.noneTitle")} description={t("ps.noneDesc")} />
      ) : (
        <div className="port-grid">
          {openPorts.map((p) => (
            <div key={p} className="fk-panel fk-panel--port is-open" style={{ textAlign: "center", padding: "12px 10px" }}>
              ...
              <div className="port-grid__status" style={{ fontSize: 11.5, fontWeight: 600, marginTop: 4, color: "var(--success)" }}>{t("ps.openStatus")}</div>
            </div>
          ))}
        </div>
      )}
